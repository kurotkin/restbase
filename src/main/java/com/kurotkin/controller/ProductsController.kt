...
@RequestMapping("products")                     // Префикс маршрута для всех экшенов
class ProductsController (private val productService: ProductService){
    ...
    @ResponseStatus(HttpStatus.CREATED)         // Cпецифический HttpStatus при успешном ответе
    ...
    @GetMapping("{id}")                         // GET запрос с параметром в url (http://localhost/products/{id})
    @ResponseStatus(HttpStatus.FOUND)
    fun read(@PathVariable id: Long) = productService.get(id) // id типа Long и передаем его в метод get сервиса
